...
step3() {
  ...
  local target_genes_file="${GENE_LIST}"
  local mm2opt="-x lr:hq -t ${mm2thread} -e10000 -k19 -w19 -g10k \
...
step4() {
  ...
  local FILTER_SCRIPT="${SCRIPT_DIR}/src/filter_paf_nano.py"
  local SEARCH_SCRIPT="${SCRIPT_DIR}/src/search_template_nano.py"
...
step5() {
  ...
  call_variants="${SCRIPT_DIR}/src/call_variants_nano.py"
