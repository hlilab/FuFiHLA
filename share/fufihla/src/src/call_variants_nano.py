...
def _is_candidate_better(c0, c1):
    ...
    # Decide better: lower adjusted mismatches, then lower adjusted NM.
    if mismatch1_adj < mismatch0_adj:
        return True
    if mismatch1_adj == mismatch0_adj and nm1_adj < nm0_adj:
        return True
    return False

best_for_read = {}
best_for_read_per_gene = {} 
...
for read, cand_list in dat.items():
    ...
    for cand in cand_list:
        g = cand[0]
        by_gene.setdefault(g, []).append(cand)

    per_gene_best = {}
    for g, clist in by_gene.items():
        best_candidate_g = clist[0]
        for candidate in clist[1:]:
            if _is_candidate_better(best_candidate_g, candidate):
                best_candidate_g = candidate
        per_gene_best[g] = best_candidate_g

    # Store per-gene winners
    best_for_read_per_gene[read] = per_gene_best

    winners = list(per_gene_best.values())
    overall_best = winners[0]
    for candidate in winners[1:]:
        if _is_candidate_better(overall_best, candidate):
            overall_best = candidate
    best_for_read[read] = overall_best

with open(os.path.join(OUTDIR, "read_assignment.txt"), "w") as f:
    for read, per_gene_best in best_for_read_per_gene.items():
        alleles = [cand[1] for cand in per_gene_best.values()]
        f.write(f"{read}\t{','.join(alleles)}\n")

# Build allele → reads mapping
allele_to_reads = defaultdict(list)
for read, per_gene_best in best_for_read_per_gene.items():
    for gene, cand in per_gene_best.items():
        allele = cand[1]
        allele_to_reads[allele].append(read)
